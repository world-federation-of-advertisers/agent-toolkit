...
4. **For each advertiser (Meta side):**
   ...
   d. **AI text grouping** (no config or unmatched): two-tier token clustering over `Campaign Name + Ad Account Name + Creative Titles + Creative Bodies` (creative fields weighted 2× over names, since campaign names are often cryptic codes). All comparison is case- and accent-insensitive. Pipeline order:
      ...
      **vi. Display normalization** — the AI label is the single top discriminative token, mapped to a **canonical multi-word label** so Meta-side groups merge with TV-side ones instead of fragmenting. `build_canonical_labels` indexes this advertiser's TV `Brand parts[1]` values and config `groups[].display_name`s into a `token → label` map; if the winning token resolves there *unambiguously* (e.g. `alpha` → `Widget Alpha` from a TV brand), that full label is used, otherwise the token is title-cased (`alpha` → `Alpha`). A token that maps to more than one label is dropped (falls back to title-case) so a wrong merge is never forced. This is what lets a Meta-side AI group and the matching TV-side `Widget Alpha` collapse into one L2.
